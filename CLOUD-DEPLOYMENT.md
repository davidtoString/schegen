...
- Only `/health` (minimal status) and CSS are public. The app remains a trusted-admin tool with powerful network/WordPress access, not a hardened public signup service. It has no MFA or individual team roles. Do not share the administrator account with untrusted users.
