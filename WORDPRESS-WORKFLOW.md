...
1. Upload or update `wordpress/schema-workspace.zip` version **1.2.0** under **Plugins → Add New → Upload Plugin** and activate it on the target site. Its source is `wordpress/schema-workspace.php`.
...
Both `wp_posts`, `alpha_posts`, and `client_b_42_posts`-style database prefixes are WordPress's concern, not app settings — register each independently hosted site or multisite subsite with its own canonical URL and credentials. Separate subdirectory installations work when their REST root is `<registered URL>/wp-json/`. Networks with domain mapping need the final canonical URL. A WordPress multisite network itself has not been integration-tested.
...
Choosing **Delete existing schema** on the Action step always removes the app graph and suppresses Rank Math output — no generated graph is required, and there is no further policy choice to make; removal is previewed automatically for the pages you selected. Choosing **Create new schema** instead opens an **"If a page already has schema"** selector on the Review step:
...
On the Apply step, open **Review** on each row, approve the before/after checkbox, then click **Apply reviewed changes** (or **Remove schema from selected pages** for the delete action). A fresh matching preview is required; changed schema or policy triggers an automatic re-preview.
...
- **Saved but unverified:** confirm the connector plugin is active and Rank Math's Schema module is active, purge caches and verify again. Transformed nodes may not match even if semantically equivalent.
